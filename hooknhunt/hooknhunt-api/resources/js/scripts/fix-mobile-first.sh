...
# mobile-first PWA compliance across Media, Procurement, and Finance modules
#
# Usage: ./fix-mobile-first.sh [module]
#   module: 'all' (default), 'media', 'procurement', 'finance'
#
# Author: Claude Code
# Date: 2026-01-21
##############################################################################
...
convert_inline_styles() {
    ...
    # Min-height replacements (more complex)
    if sed -i '' 's/style={{ minHeight: '"'"'calc(.*?)'"'"' }}/className="min-h-[\1]"/g' "$file" 2>/dev/null; then
        ((changes++))
    fi

    # Max-width replacements
    if sed -i '' 's/style={{ maxWidth: \([0-9]*\) }}/mw={\1}/g' "$file" 2>/dev/null; then
        ((changes++))
    fi

    # Min-width replacements
    if sed -i '' 's/style={{ minWidth: \([0-9]*\) }}/minw={\1}/g' "$file" 2>/dev/null; then
        ((changes++))
    fi
...
# Check for remaining mobile-first issues
check_compliance() {
    local file="$1"
    local issues=0

    # Check for Mantine size props (should use responsive classes)
    ...
    # Check for hardcoded strings (heuristic - should use t())
    # Skip if file already imports useTranslation
    if ! grep -q 'useTranslation' "$file" 2>/dev/null; then
        # Don't flag hardcoded strings if no i18n setup
        :
    fi
...
process_file() {
    ...
    local module_name=$(echo "$file" | grep -oE "(cms|procurement|finance)" | head -1)
    ...
    local remaining=$(check_compliance "$file")
    if [ $remaining -gt 0 ]; then
        echo "    ⚠ $remaining remaining issue(s) to fix manually"
    fi
}
...
main() {
    ...
    case "$module" in
        ...
        *)
            ...
            echo ""
            print_info "Valid options: all, media, procurement, finance"
            exit 1
            ;;
    esac
