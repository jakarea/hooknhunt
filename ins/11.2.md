...
* **`updateStatus(Request $request, $id)` (The Engine):**
    ...
    * **Logic Switch:**
        ...
        * If status `received_hub`: Validate & save `total_weight`, `extra_cost_global`. Accept optional `items` array to update `lost_quantity`.
        * If status `completed`:
...
* **`calculateAndFinalize($order)` (Private Logic):**
    * Loop through each item:
        ...
        4.  **Lost Item Logic:**
            * `effective_qty` = `quantity` - `lost_quantity`.
