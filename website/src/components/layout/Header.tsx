...
export default function Header() {
      ...
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          if (scrollDifference > 5) {
            ...
            // Determine scroll direction with threshold to prevent rapid changes
  ...
  return (
    <header className="sticky top-0 z-50 bg-white dark:bg-[#1a1a1a] shadow-sm transition-all duration-500 ease-in-out">
      {/* Top Bar */}
      <div
        className={`bg-[#bc1215] text-white overflow-hidden transition-all duration-500 ease-in-out ${
          scrollDirection === 'down' && isScrolled ? 'max-h-0 opacity-0' : 'max-h-20 opacity-100'
        }`}
      >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-2 py-0.5 text-xs font-medium  transition-colors ${
                    i18n.language === 'bn' ? 'bg-white text-[#bc1215]' : 'bg-transparent hover:bg-white/20'
                  }`}
                >
                  বাংলা
                </button>
                <button
                  ...
                  className={`px-2 py-0.5 text-xs font-medium  transition-colors ${
                    i18n.language === 'en' ? 'bg-white text-[#bc1215]' : 'bg-transparent hover:bg-white/20'
                  }`}
                >
                  EN
                </button>
      ...
      {/* Main Header */}
      <div
        className={`bg-white dark:bg-[#1a1a1a] border-b border-gray-200 dark:border-gray-800 transition-all duration-500 ease-in-out ${
          scrollDirection === 'down' && isScrolled ? 'py-3' : 'py-5'
        }`}
      >
        <div className="max-w-[1344px] mx-auto px-4 lg:px-8 xl:px-12">
          <div className="flex items-center justify-between gap-8">
            ...
            <Link href="/" className="flex-shrink-0">
              <Image
                ...
                className={`w-auto transition-all duration-500 ease-in-out ${
                  scrollDirection === 'down' && isScrolled ? 'h-10' : 'h-14'
                }`}
                priority
              />
            </Link>
            ...
            <div className="flex items-center gap-6">
              <Link href="/account" className="hidden sm:flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-[#bc1215] dark:hover:text-[#bc1215] transition-colors">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span className="text-[15px] font-medium">{t('header.account')}</span>
              </Link>
      ...
      <div
        className={`hidden lg:block bg-[#f5f5f5] dark:bg-[#1a1a1a] border-b border-gray-200 dark:border-gray-800 transition-all duration-500 ease-in-out overflow-hidden ${
          scrollDirection === 'down' && isScrolled ? 'max-h-0 opacity-0' : 'max-h-20 opacity-100'
        }`}
      >
        <div className="max-w-[1344px] mx-auto px-4 lg:px-8 xl:px-12">
          <nav className="flex items-center justify-start gap-8">
            ...
            <Link href="/products?category=rods" className="py-4 text-[15px] font-medium text-gray-800 dark:text-gray-200 hover:text-[#bc1215] dark:hover:text-[#bc1215] transition-colors relative group">
              {t('nav.rods')}
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#bc1215] transform scale-x-0 group-hover:scale-x-100 transition-transform"></span>
            </Link>

            <Link href="/products?category=reels" className="py-4 text-[15px] font-medium text-gray-800 dark:text-gray-200 hover:text-[#bc1215] dark:hover:text-[#bc1215] transition-colors relative group">
              {t('nav.reels')}
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#bc1215] transform scale-x-0 group-hover:scale-x-100 transition-transform"></span>
            </Link>

            <Link href="/products?category=lures" className="py-4 text-[15px] font-medium text-gray-800 dark:text-gray-200 hover:text-[#bc1215] dark:hover:text-[#bc1215] transition-colors relative group">
              {t('nav.lures')}
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#bc1215] transform scale-x-0 group-hover:scale-x-100 transition-transform"></span>
            </Link>

            <Link href="/products?category=lines" className="py-4 text-[15px] font-medium text-gray-800 dark:text-gray-200 hover:text-[#bc1215] dark:hover:text-[#bc1215] transition-colors relative group">
              {t('nav.lines')}
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#bc1215] transform scale-x-0 group-hover:scale-x-100 transition-transform"></span>
            </Link>

            <Link href="/products?category=accessories" className="py-4 text-[15px] font-medium text-gray-800 dark:text-gray-200 hover:text-[#bc1215] dark:hover:text-[#bc1215] transition-colors relative group">
              {t('nav.accessories')}
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#bc1215] transform scale-x-0 group-hover:scale-x-100 transition-transform"></span>
            </Link>
