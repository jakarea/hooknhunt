...
export default function EditEmployeePage() {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        setRoles(rolesData.map((r: any) => ({ value: String(r.id), label: r.name })))

        // Fetch departments
        const deptRes = await api.get('/hrm/departments')
        const deptData = deptRes.data.data || []
        setDepartments(deptData.map((d: any) => ({ value: String(d.id), label: d.name })))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Always include profile fields (backend will decide to create/update)
      payload.department_id = departmentId ? parseInt(departmentId) : null
      payload.designation = designation || null
      // Format dates for MySQL (YYYY-MM-DD)
      payload.joining_date = formatDateForMySQL(joiningDate)
      // base_salary is required by database (NOT NULL), so don't send null
      payload.base_salary = baseSalary !== null && baseSalary !== undefined ? baseSalary : 0
      payload.address = address || null
      payload.city = city || null
      // Format dates for MySQL (YYYY-MM-DD)
      payload.dob = formatDateForMySQL(dob)
